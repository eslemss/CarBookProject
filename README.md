...
"📝 Akıllı Rezervasyon Formu ve Dinamik Veri Yönetimi: Sistem mimarisindeki Kiralama sınıfının (class) somutlaştığı ana veri giriş arayüzüdür. Bu ekranda, kullanıcı deneyimini (UX) optimize etmek amacıyla Kimlik Doğrulama (Authentication) sisteminden gelen kullanıcı bilgileri (Ad, Soyad, E-posta) input alanlarına otomatik olarak yansıtılmaktadır (Initial Value Binding).

Otomatik dolan bu bilgiler, esneklik sağlamak adına kullanıcı tarafından manuel olarak da güncellenebilmektedir. Formun çekirdek yapısı; kullanıcının seçtiği araç verilerini tutarlı bir şekilde işlerken, 'Yaş' ve 'Ehliyet Numarası' gibi alanlar üzerinden arka plandaki İş Mantığı (Business Logic) kurallarını çalıştırır. Girilen veriler doğrulandıktan sonra, sistemin Kiralama_Olustur() metodu tetiklenerek süreç güvenli bir şekilde veritabanına aktarılır."
<img width="658" height="890" alt="rezervasyonformu" src="https://github.com/user-attachments/assets/f361ffcd-cc4d-4e5f-bf12-2aa23bcb80da" />
